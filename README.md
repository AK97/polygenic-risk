...
**This program is built in both Python and Rust**, using the same data files and producing the same output.
...
- `src/main.rs` is the Rust version of the program. Loads GWAS and Genome sample from file, then performs PRS calculation and prints results.

Running it
----------
1) Install Python and dependencies, or Install Rust:
```
python -m venv venv
...
- Genome data: `data/<name>.bed` with columns `chromosome`, `start`, `end`, `genotype`. The default file used is `data/human1_hg38.bed`.
3) Run the script (Python or Rust):  
`> python3 main.py`  
or  
`> cargo run`  
It will report how many SNPs are aligned and pass the p-value threshold, and the resulting PRS.
